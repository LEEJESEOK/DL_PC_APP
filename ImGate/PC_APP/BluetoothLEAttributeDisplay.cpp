...
namespace winrt::PC_APP::DisplayHelpers
{
	hstring GetServiceName(GattDeviceService const& service)
	{
		...
		if (TryParseSigDefinedUuid(uuid, shortId))
		{
			...
			const static std::map<uint32_t, const wchar_t*> customServiceIds =
			{
				{ 0x0001, L"Nordic UART" },
			};
			auto it = customServiceIds.find(shortId);
			if (it != customServiceIds.end())
			{
				return it->second;
			}
		}
	...
	hstring GetCharacteristicName(GattCharacteristic const& characteristic)
	{
		...
		if (TryParseSigDefinedUuid(uuid, shortId))
		{
			...
			{
				{ 0x0000, L"None" },
				{ 0x0002, L"RX Characteristic" },
				{ 0x0003, L"TX Characteristic" },
			...
			const static std::map<uint32_t, const wchar_t*> customCharacteristicIds =
			{
				{ 0x0002, L"RX Characteristic" },
				{ 0x0003, L"TX Characteristic" },
			};

			auto it = customCharacteristicIds.find(shortId);
			if (it != customCharacteristicIds.end())
			{
				return it->second;
			}
		}
